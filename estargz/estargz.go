...
func NewWriterLevel(w io.Writer, compressionLevel int) *Writer {
	return NewWriterWithCompressor(w, &GzipCompressor{compressionLevel})
}
...
func newReader(sr *io.SectionReader, opt ...OpenOption) (*Reader, error) {
	...
	for _, d := range decompressors {
		...
		r, err := parseTOC(d, sr, tocOffset, tocSize, maybeTocBytes)
		if err != nil {
			allErr = append(allErr, err)
			continue
		}
...
func parseTOC(d Decompressor, sr *io.SectionReader, tocOff, tocSize int64, tocBytes []byte) (*Reader, error) {
	if len(tocBytes) > 0 {
